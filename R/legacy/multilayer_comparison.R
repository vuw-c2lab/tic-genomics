...
theData <- "full-country_data"
for(j in c(20,100)){
  res <- data.frame(cntry=character(0),diamG2=numeric(0),avgspathG2=numeric(0),modG2=numeric(0),diamG3=numeric(0),avgspathG3=numeric(0),modG3=numeric(0),diamG4=numeric(0),avgspathG4=numeric(0),modG4=numeric(0))
  fn <- paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",j,"/clustersNo.csv")
  ...
  for(i in folders){
    ...
    write.csv(as.data.frame(as.matrix(as_adjacency_matrix(gg,attr = "Weight"))),paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",j,"/",i,"_g0.csv"),col.names = T,row.names = T)
    
    #2nd layer
    links <- read_csv(paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",i,"/metadata/links.csv"))
    nodes <- read_csv(paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",i,"/metadata/nodes.csv"))
    colnames(links) <- c("id1","id2","label")
    ...
    cluslinks <- links[which(grepl("Location: ",links$label) | grepl("Travel: ",links$label)),]
    ...
    write.csv(as.data.frame(as.matrix(as_adjacency_matrix(g))),paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",j,"/",i,"_g2.csv"),col.names = T,row.names = T)
    ...
    dislinks <- links[which(grepl("Age: ",links$label) | grepl("Gender: ",links$label)),]
    ...
    write.csv(as.data.frame(as.matrix(as_adjacency_matrix(g2))),paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",j,"/",i,"_g3.csv"),col.names = T,row.names = T)
    ...
    dislinks <- links[which(grepl("Age: ",links$label)),]
    dislinks$id1 <- paste0(dislinks$id1," - ",names[dislinks$id1],"-L4")
    dislinks$id2 <- paste0(dislinks$id2," - ",names[dislinks$id2],"-L4")
    colnames(dislinks) <- c("Source","Target","Label")
    
    g3 <- graph_from_data_frame(as.data.frame(dislinks),directed = T,vertices = tmpNodes[,5])
    V(g3)$label <- V(g3)$name
    write.csv(as.data.frame(as.matrix(as_adjacency_matrix(g3))),paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",j,"/",i,"_g4.csv"),col.names = T,row.names = T)
    res<-rbind(res,data.frame(cntry=i,diamG2=diameter(g),avgspathG2=mean_distance(g,unconnected = T),modG2=modularity(g,membership = cluster_walktrap(g)$membership),diamG3=diameter(g2),avgspathG3=mean_distance(g2,unconnected = T),modG3=modularity(g2,membership = cluster_walktrap(g2)$membership),diamG4=diameter(g3),avgspathG4=mean_distance(g3,unconnected = T),modG4=modularity(g3,membership = cluster_walktrap(g3)$membership)))
    
    # combine multiple metadata layers
    tmpNodes <- nodes
    tmpNodes$id <- tmpNodes$genenames
    tmpNodes$id <- paste0(tmpNodes$id,"-L4")
    
    dislinks <- links[which(grepl("Age: ",links$label) | grepl("Location: ",links$label) | grepl("Travel: ",links$label) | grepl("Gender: ",links$label)),]
    dislinks$id1 <- paste0(dislinks$id1," - ",names[dislinks$id1],"-L4")
    dislinks$id2 <- paste0(dislinks$id2," - ",names[dislinks$id2],"-L4")
    colnames(dislinks) <- c("Source","Target","Label")
    
    gcombined <- graph_from_data_frame(as.data.frame(dislinks),directed = T,vertices = tmpNodes[,5])
    V(gcombined)$label <- V(gcombined)$name
    write.csv(as.data.frame(as.matrix(as_adjacency_matrix(gcombined))),paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",j,"/",i,"_gcombined.csv"),col.names = T,row.names = T)
    
    # nucleotide layer
    links <- read_csv(paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",i,"/nucleotides/links.csv"))
    nodes <- read_csv(paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",i,"/nucleotides/nodes.csv"))
    colnames(links) <- c("id1","id2","label")
    nodes$genenames <- paste(nodes$id," - ",names,sep="")
    tokenCount <- plyr::count(links$label)
    tokenCount$x <- as.character(tokenCount$x)
    ...
    write.csv(as.data.frame(as.matrix(as_adjacency_matrix(gnu,attr = "Weight"))),paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",j,"/",i,"_g1.csv"),col.names = T,row.names = T)
    ...
    write.tree(gp,paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",j,"/",i,"_tree_nucleotides_tokens_",j,"_0_setmetric.nwk"))
    write_csv(data.frame(cntry=i,clusters=max(walktrap.community(gg)$membership),branchLengthAvg=mean(gp$edge.length),branchLengthSd=sd(gp$edge.length)),paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",j,"/clustersNo.csv"),append = T)
  }
  write_csv(res,paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",j,"/metagraphs.csv"))
}

#### the tree ------
foo<-"Russia"
countryDs <- readRDS(paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",foo,"_aligned.rda"))
names <- countryDs$nam
links <- read_csv(paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",foo,"/nucleotides/links.csv"))
nodes <- read_csv(paste0("~/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/tic/output/",theData,"/",foo,"/nucleotides/nodes.csv"))
colnames(links) <- c("id1","id2","label")

nodes$genenames <- paste(nodes$id," - ",names,sep="")
tokenCount <- plyr::count(links$label)
tokenCount$x <- as.character(tokenCount$x)
tokensOut <- tokenCount[which(tokenCount$freq>(20*(nrow(nodes)/100)) | tokenCount$freq<=(0*(nrow(nodes)/100))),1]
agg_links <- aggregate(label ~ .,links[which(!links$label %in% tokensOut),],paste, collapse = ", ")
#links<-links[which(grepl("\\+1",links$label)),]
#agg_links <- aggregate(label ~ .,links,paste, collapse = ", ")
...
phycanv_gg
write.tree(gp,paste0(foo,"_tree_nucleotides_tokens_20_0_setmetric.nwk"))
