...
prep <- data.frame(seq=character(0),dats=numeric(0),nams=character(0),metatoks=character(0),stringsAsFactors = F)
...
for(fil in fils){
  ...
  if(dist<=5000 & max(attributes(match)$match.length < 100)){
    if(length(which(metaNZ$Accession==nam))>0){
      if(!is.na(metaNZ$onsetdt[which(metaNZ$Accession==nam)]) | !is.na(metaNZ$Date.Collected[which(metaNZ$Accession==nam)])){
        if(!is.na(metaNZ$onsetdt[which(metaNZ$Accession==nam)])){
          dat <- ifelse(is.na(lubridate::mdy(metaNZ$onsetdt[which(metaNZ$Accession==nam)])),toString(lubridate::ymd(metaNZ$onsetdt[which(metaNZ$Accession==nam)])),toString(lubridate::mdy(metaNZ$onsetdt[which(metaNZ$Accession==nam)])))
        } else {
          dat <- toString(metaNZ$Date.Collected[which(metaNZ$Accession==nam)])
        }
        prep <- rbind(prep,data.frame(seq=seq,dats=dat,nams=nam,metatoks=metaNZ$tokenised[which(metaNZ$Accession==nam)]))
      }
    }
  }
}

prep <- prep[which(!is.na(prep$dats)),]
...
for(i in 1:nrow(subset.ds)){
  if(!grepl("New Zealand",subset.ds$nams[i]) & !is.na(subset.ds$nams[i])){
    prep <- rbind(prep,data.frame(seq=subset.ds$seq[i],dats=subset.ds$dats[i],nams=subset.ds$nams[i],metatoks=subset.ds$metatoks[i]))
  }
}
...
write_csv(as.data.frame(prep$seq),"ordered_nz_plus.txt",col_names = F)
