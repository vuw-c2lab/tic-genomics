...
import sys
sys.path.append('/Users/mlr/OneDrive - Victoria University of Wellington - STAFF/Python Scripts/')
...
def tokenise_line(line: str) -> List[str]:
    ...
    for idx in range(len(line)):
        codon = line[idx]
        if len(codon) == 1 and 'n' not in codon and '-' not in codon:
            token = f'pos{idx + 1}#{codon}'
